$(document).ready(function () {
	...
	$('#button_Go').click(function () {
		...
		if(date) {//проверяем заполнена ли дата
			if(name) {//проверяем заполнено ли имя
				let randomIndex = Math.floor(Math.random() * 14),//генерируем рандомное число
					message = 'Привет, '+name+' Ваше гадание на '+date+' готово<br>'+answer[randomIndex];//составляем сообщение
